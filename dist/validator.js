...
exports.BrainfileValidator = void 0;
...
class BrainfileValidator {
    ...
    static validate(board) {
        ...
        if (board.archive !== undefined) {
            if (!Array.isArray(board.archive)) {
                ...
                board.archive.forEach((task, index) => {
     ...
     */
    static validateColumn(column, path) {
        const errors = [];
        if (!column) {
     ...
     * @returns Array of validation errors
     */
    static validateSubtask(subtask, path) {
        const errors = [];
        if (!subtask) {
     ...
     * @returns Array of validation errors
     */
    static validateRules(rules, path) {
        const errors = [];
        if (!rules) {
     ...
     */
    static validateBrainfile(data, filename) {
        if (!data) {
            return {
                valid: false,
        ...
        // Infer type
        const type = (0, inference_1.inferType)(data, filename);
        ...
        if (type === types_1.BrainfileType.BOARD || (!type && data.columns)) {
            errors.push(...this.validate(data).errors);
        }
        else if (type === types_1.BrainfileType.JOURNAL || data.entries) {
            errors.push(...this.validateJournal(data).errors);
        }
     ...
     * @returns Array of validation errors
     */
    static validateJournalEntry(entry, path) {
        const errors = [];
        if (!entry) {
...
exports.BrainfileValidator = BrainfileValidator;
