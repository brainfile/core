...
exports.taskToCompletedRef = taskToCompletedRef;
exports.ledgerRecordToCompletedRef = ledgerRecordToCompletedRef;
/**
 * Project a full Task (from board .md or log .md) into a CompletedTaskRef.
 */
function taskToCompletedRef(task) {
    const ref = {
        id: task.id,
        title: task.title,
    };
    if (task.description)
        ref.description = task.description;
    if (task.assignee)
        ref.assignee = task.assignee;
    if (task.priority)
        ref.priority = task.priority;
    if (task.tags?.length)
        ref.tags = task.tags;
    if (task.relatedFiles?.length)
        ref.relatedFiles = task.relatedFiles;
    ...
    const metrics = task.contract?.metrics;
    const metricPaths = metrics?.deliverablePaths;
    if (Array.isArray(metricPaths) && metricPaths.length > 0) {
        const additional = metricPaths.filter((p) => typeof p === 'string' && p.trim().length > 0);
 ...
 */
function ledgerRecordToCompletedRef(record) {
    const ref = {
        id: record.id,
        title: record.title,
    };
    if (record.summary)
        ref.description = record.summary;
    if (record.assignee)
        ref.assignee = record.assignee;
    if (record.priority)
        ref.priority = record.priority;
    if (record.tags?.length)
        ref.tags = record.tags;
    if (record.relatedFiles?.length)
        ref.relatedFiles = record.relatedFiles;
    if (record.dependsOn?.length)
        ref.dependsOn = record.dependsOn;
